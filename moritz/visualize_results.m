function visualize_results(pair, results_path, figures_path)
    ...
    %% Load the results
    name_file = fullfile(results_path, ['OVO_', pair, '.mat']);
    ...
    if strcmp(pair, 'lookalike-animal')
        name = 'Trained on lookalike and Tested on Animals';
    elseif strcmp(pair, 'lookalike-object')
        name = 'Trained on lookalike and Tested on Inanimate Objects';
    elseif strcmp(pair, 'animal-object')
        name = 'Trained on Animals and Tested on Inanimate';
    end
    title(name);
